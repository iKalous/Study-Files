...
int n,m,a[N];
ll p;
struct node {int q,w; node(){q = 0,w = 0; } };
    node operator + (node x,node y) 
	{ 
		node c;
	/*	c.q = x.q + y.q;
		c.w = x.w + y.w; */
		c = (node){ x.q + y.q, x.w + y.w };
		return c;
	}
    ll operator * (ll a,ll b) { return a * b % p; }
    ll operator += (const ll &b) { return (this + b) % p; }
    ll operator *= (const ll &b) { return this * b % p; }
...
struct SGT
{
    ...
    {
        sum(ls(x)) = (sum(ls(x)) * tag_mul(ls(x)) + (r(ls(x)) - l(ls(x)) + 1) * tag_add(ls(x)));
        sum(rs(x)) = (sum(rs(x)) * tag_mul(rs(x)) + (r(rs(x)) - l(rs(x)) + 1) * tag_add(rs(x)));

        tag_mul(ls(x)) *= tag_mul(x), tag_mul(rs(x)) *= tag_mul(x);
        tag_add(ls(x)) = tag_mul(x) * tag_add(ls(x)) + tag_add(x);
        tag_add(rs(x)) = tag_mul(x) * tag_add(rs(x)) + tag_add(x);
    ...
    {
        if(l <= l(x) && r(x) <= r) 
            { tag_add(x) += (ll)k; sum(x) += (ll)(r(x) - l(x) + 1ll) * k; return; }
        pushdown(x);
        int mid = (l(x) + r(x)) >> 1;
    ...
    {  
        if(l <= l(x) && r(x) <= r)
            { tag_add(x) *= (ll)k, tag_mul(x) *= (ll)k, sum(x) *= (ll)k; return; }
        pushdown(x);
        int mid = (l(x) + r(x)) >> 1;
